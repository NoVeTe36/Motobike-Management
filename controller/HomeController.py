...
class HomeController(Controller):
    ...
    def __init__(self,root,user):
        self.homeView = self.loadView("Home",root,user)
        self.reverse = False
        #self.brand = Brand()
...
# ----------------------------------------------------------------
    ...
    def on_select(self, event):
        ...
        if selected_item is not None:
            ...
            edit_view = self.loadEdit("edit",tk.Tk(), values, self.frame, self.productFrame)
            # reset selected item
    ...
    def order_delete(self, event):
        ...
        if confirm:
            for item in selected_items:
                item_values = event.widget.item(item, "values")
                self.order.delete_order(item_values[1], item_values[3], item_values[4], item_values[5])
                event.widget.delete(item)
...
#DISPLAY_BRAND_LIST
    ...
    def btnUpdate_Product(self, fields, fields_old,frame, contentFrame, productFrame):
        ...
        if response == 1:
            ...
            self.tree1.destroy()
            self.showTreeView_ProductList(contentFrame, productFrame, self.searchContent, self.filterContent, self.categoryContent, self.brandContent)
            frame.destroy()
        else:
            messagebox.showerror("Error", "Update product failed")
        

    def showTreeView_ProductList(self, contentFrame, productFrame, searchContent='', filterContent='Sort', categoryContent='Category', brandContent='Brand'):
    ...
    # ORDER LIST
    def showTreeView_OrderProductList(self, contentFrame, productEntry, productContent):
        style = ttk.Style()
        ...
        tree.column("quantity", width=120, stretch=False, anchor="center")
        tree.heading("#0", text="No.", anchor="center")
        ...
        if productContent == "":
            ...
            for i in range(len(product_list)):
                if productContent.lower() in product_list[i][0].lower():
                    tree.insert('', 'end', text=i+1, values=(product_list[i][0], product_list[i][1], product_list[i][2], product_list[i][3], product_list[i][4], product_list[i][5]))
        list = None
    ...
    def btnAdd_Order(self, fields, temp_frame, contentFrame, productEntry, tree):
        ...
        # Reset order list
        tree.destroy()
        self.showTreeView_OrderProductList(contentFrame, productEntry, self.productContent)

        # product_list = self.product.get_product_list()
        # try:
        #     self.tree1.destroy()
        #     self.reset_ProductList(contentFrame, product_list)
        # except:
        #     pass

    def showTreeView_OrderList(self, contentFrame, productFrame, customerContent):
        style = ttk.Style()
        style.configure("Treeview", rowheight=25)
        tree = ttk.Treeview(productFrame, height=11)
        ...
        tree.column("Selling_Price_M", width=160, stretch=False, anchor="center")
        tree.heading("#0", text="No.", anchor="center")
        ...
        if customerContent == "":
            ...
            for i in range(len(order_list)):
                if customerContent.lower() in order_list[i][1].lower():
                    tree.bind("<Delete>", self.order_delete)
                    tree.insert('', 'end', text=i+1, values=(order_list[i][0], order_list[i][1], order_list[i][2], order_list[i][3], order_list[i][4], order_list[i][5], order_list[i][6], order_list[i][7], order_list[i][8]))
        # Scrollbar
        scrollBarY = ttk.Scrollbar(contentFrame, orient="vertical", command=tree.yview)
    ...
    # ----------------user info------------------------
    def take_info(self, username):
        return self.user.info(username)
    
    def edit_info(self, fields, username):
        return self.user.edit(fields, username)

    def get_login_history(self, username):
        return self.user.get_login_history(username)
    
    def openLoginPage(self):
        self.loginPage = MainApplication()
