...
#Filter display
    def filterDropCallback(self,contentFrame,filterDrop):
        ...
        style.configure("Treeview", rowheight=25)
        tree = ttk.Treeview(contentFrame, height=11)
        tree["columns"] = ("product", "brand", "category", "length_mm", "width_mm", "height_mm", "mass_kg", "fuel_capacity", "fuel_consumption_100km", "engine_type", "Maximize_Efficiency_kW_minute", "color", "Selling_Price_M", "quantity" )
        tree.column("#0", width=50, stretch=False)
        tree.column("product", width=250, stretch=False)
        tree.column("brand", width=100, stretch=False)
        tree.column("category", width=100, stretch=False)
